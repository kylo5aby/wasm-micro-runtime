...
TEST_F(WasmExternTypeTest, wasm_extern_type_FunctionExtern_ReturnsCorrectExternType) {
    ...
    wasm_functype_t* retrieved_functype = wasm_externtype_as_functype(externtype);
...
TEST_F(WasmExternTypeTest, wasm_extern_type_FunctionExternWithParams_ReturnsCorrectExternType) {
...
TEST_F(WasmExternTypeTest, wasm_extern_type_GlobalExtern_ReturnsCorrectExternType) {
    // Create global type
    ...
    // Convert back to verify correctness
    wasm_globaltype_t* retrieved_globaltype = wasm_externtype_as_globaltype(externtype);
    ASSERT_NE(nullptr, retrieved_globaltype);
    ASSERT_EQ(WASM_I32, wasm_valtype_kind(wasm_globaltype_content(retrieved_globaltype)));
...
TEST_F(WasmExternTypeTest, wasm_extern_type_ConstGlobalExtern_ReturnsCorrectExternType) {
    ...
    wasm_globaltype_t* retrieved_globaltype = wasm_externtype_as_globaltype(externtype);
    ASSERT_NE(nullptr, retrieved_globaltype);
    ASSERT_EQ(WASM_F64, wasm_valtype_kind(wasm_globaltype_content(retrieved_globaltype)));
...
TEST_F(WasmExternTypeTest, wasm_extern_type_MemoryExtern_ReturnsCorrectExternType) {
    // Create memory type
...
TEST_F(WasmExternTypeTest, wasm_extern_type_TableExtern_ReturnsCorrectExternType) {
    // Create table type
    ...
    // Convert back to verify correctness
    wasm_tabletype_t* retrieved_tabletype = wasm_externtype_as_tabletype(externtype);
    ASSERT_NE(nullptr, retrieved_tabletype);
    
    const wasm_limits_t* retrieved_limits = wasm_tabletype_limits(retrieved_tabletype);
    ASSERT_NE(nullptr, retrieved_limits);
...
TEST_F(WasmExternTypeTest, wasm_extern_type_TableExternWithFuncRef_ReturnsCorrectExternType) {
...
TEST_F(WasmExternTypeTest, wasm_extern_type_InvalidKind_ReturnsNullAndLogsWarning) {
    ...
    // we'll test this by creating a valid extern and verifying that all valid kinds work
    // The default case would only be hit with corrupted data or future extensions
...
TEST_F(WasmExternTypeTest, wasm_extern_type_MultipleCalls_ReturnConsistentResults) {
